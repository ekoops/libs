...
static __always_inline int __call_toctou_filler(void *ctx, long syscall_id) {
	...
	call_toctou_filler(ctx, ctx, evt_type, drop_flags, socketcall_syscall_id);
	return 0;
}

#define TOCTOU_FILLER_RAW(x)                                                                 \
	__bpf_section("tracepoint/filler/toctou/" #x) static __always_inline int bpf_toctou_##x( \
	        void *ctx)

#define TOCTOU_ENTER_PROBE(tp_name, prog_name, ctx_type)                                \
	__bpf_section("tracepoint/syscalls/sys_enter_" #tp_name) static __always_inline int \
	        bpf_toctou_caller_##prog_name(ctx_type *ctx) {                              \
		return __call_toctou_filler(ctx, ctx->__syscall_nr);                            \
	}                                                                                   \
                                                                                        \
	static __always_inline int __bpf_toctou_##prog_name(struct filler_data *data);      \
                                                                                        \
	__bpf_section("tracepoint/filler/toctou/" #prog_name) static __always_inline int    \
	        bpf_toctou_##prog_name(ctx_type *ctx) {                                     \
		...
		if(res == PPM_SUCCESS) {                                                        \
			...
			res = __bpf_toctou_##prog_name(&data);                                      \
		}                                                                               \
                                                                                        \
		if(res == PPM_SUCCESS) {                                                        \
		...
		bpf_tail_call(ctx, &toctou_tail_map, PPM_FILLER_terminate_filler);              \
		bpf_printk("Can't tail call terminate TOCTOU filler\n");                        \
		return 0;                                                                       \
	}                                                                                   \
                                                                                        \
	static __always_inline int __bpf_toctou_##prog_name(struct filler_data *data)

TOCTOU_FILLER_RAW(terminate_filler) {
...
TOCTOU_ENTER_PROBE(connect, sys_connect_e, struct sys_enter_connect_args) {
...
TOCTOU_ENTER_PROBE(creat, sys_creat_e, struct sys_enter_creat_args) {
...
TOCTOU_ENTER_PROBE(open, sys_open_e, struct sys_enter_open_args) {
...
TOCTOU_ENTER_PROBE(openat, sys_openat_e, struct sys_enter_openat_args) {
