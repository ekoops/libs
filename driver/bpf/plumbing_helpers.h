...
static __always_inline bool drop_event(void *ctx,
                                       const bool is_toctou_probe_ctx,
                                       struct scap_bpf_per_cpu_state *state,
	...
	/* "TOCTOU probe context" means that the provided context is tracepoint-specific, and we cannot
	 * use the below logic to extract syscall arguments from it (i.e., we cannot use
	 * bpf_syscall_get_argument_from_ctx()` or similar helpers), so simply this logic.
	 * We can later add any custom logic required to handle specific situations as we implement new
	 * TOCTOU mitigation programs for other event types. */
	if(!is_toctou_probe_ctx) {
		switch(evt_type) {
...
static __always_inline void call_toctou_filler(void *ctx,
                                               void *stack_ctx,
                                               ppm_event_code evt_type,
                                               enum syscall_flags drop_flags,
                                               int socketcall_syscall_id) {
	...
	bpf_tail_call(ctx, &toctou_tail_map, filler_info->filler_id);
	bpf_printk("Can't tail call TOCTOU filler evt=%d, filler=%d\n",
	           state->tail_ctx.evt_type,
	           filler_info->filler_id);
