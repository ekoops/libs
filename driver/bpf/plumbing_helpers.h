...
static __always_inline bool drop_event(void *ctx,
	...
	/* "TOCTOU mitigation probe context" means that the provided context is tracepoint-specific, and
	 * we cannot use the below logic to extract syscall arguments from it (i.e., we cannot use
	 * bpf_syscall_get_argument_from_ctx()` or similar helpers), so simply skip this logic.
...
static __always_inline void call_ttm_filler(void *ctx,
                                            void *stack_ctx,
                                            ppm_event_code evt_type,
                                            enum syscall_flags drop_flags,
                                            int socketcall_syscall_id) {
