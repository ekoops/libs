...
static __always_inline int bpf_poll_parse_fds(struct filler_data *data, bool enter_event) {
	...
	if(read_size)
		if(bpf_probe_read_user(fds, ((read_size - 1) & SCRATCH_SIZE_MAX) + 1, (void *)val))
#else
...
static __always_inline int bpf_parse_readv_writev_bufs(struct filler_data *data,
	...
	if(copylen)
		if(bpf_probe_read_user(data->tmp_scratch,
		                       ...
		                       (void *)iovsrc))
#else
	if(bpf_probe_read_user(data->tmp_scratch, copylen & SCRATCH_SIZE_MAX, (void *)iovsrc))
#endif
...
static __always_inline int timespec_parse(struct filler_data *data, unsigned long val) {
	if(!bpf_in_ia32_syscall()) {
...
static __always_inline unsigned long bpf_get_mm_rss(struct mm_struct *mm) {
	return bpf_get_mm_counter(mm, MM_FILEPAGES) + bpf_get_mm_counter(mm, MM_ANONPAGES) +
...
parse_sockopt(struct filler_data *data, int level, int optname, void *optval, int optlen) {
	...
	case SO_SNDTIMEO_NEW:
		...
		return bpf_val_to_ring_dyn(data,
...
static __always_inline pid_t bpf_pid_nr_ns(struct pid *pid, struct pid_namespace *ns) {
...
static __always_inline struct pid **bpf_task_pid_ptr(struct task_struct *task, enum pid_type type) {
	return (type == PIDTYPE_PID) ? &task->thread_pid : &_READ(task->signal)->pids[type];
}
#endif
...
static __always_inline int __bpf_append_cgroup(struct css_set *cgroups,
	...
	int res = bpf_probe_read_kernel_str(&buf[off_bounded], SCRATCH_SIZE_HALF, subsys_name);
	if(res == -EFAULT)
		return PPM_FAILURE_INVALID_USER_MEMORY;
...
#pragma unroll MAX_CGROUP_PATHS
	for(int k = MAX_CGROUP_PATHS - 1; k >= 0; --k) {
		if(cgroup_path[k]) {
			...
			res = bpf_probe_read_kernel_str(&buf[off_bounded], SCRATCH_SIZE_HALF, cgroup_path[k]);
			if(res > 1) {
				off += res - 1;
				off_bounded = off & SCRATCH_SIZE_HALF;
			} else if(res == 1)
...
static __always_inline int bpf_append_cgroup(struct task_struct *task, char *buf, int *len) {
...
static __always_inline struct uid_gid_extent *bpf_map_id_up_max(unsigned extents,
...
static __always_inline struct uid_gid_extent *bpf_map_id_up_base(unsigned extents,
                                                                 struct uid_gid_map *map,
                                                                 uint32_t id) {
	unsigned idx;
	uint32_t first, last;
...
static __always_inline bool get_exe_writable(struct inode *inode, struct cred *cred) {
	...
	// Check if the user is capable. Even if it doesn't own the file or the read bits are not set,
	// root with CAP_FOWNER can do what it wants.
...
FILLER(proc_startupdate, true) {
	...
	exe_len =
	        bpf_probe_read_kernel_str(&data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF],
	                                  SCRATCH_SIZE_HALF,
...
FILLER(proc_startupdate_2, true) {
	...
	res = __bpf_val_to_ring(data,
	                        (unsigned long)data->tmp_scratch,
	                        cgroups_len,
	                        PT_BYTEBUF,
	                        -1,
	                        false,
...
FILLER(execve_extra_tail_1, true) {
	...
	res = bpf_push_u64_to_ring(
	        data,
	        capabilities_to_scap(((unsigned long)cap.cap[1] << 32) | cap.cap[0]));
#else
...
FILLER(execve_extra_tail_2, true) {
	...
	if(data->state->tail_ctx.evt_type != PPME_SYSCALL_EXECVE_19_X) {
		return res;
	}
	CHECK_RES(res);
...
FILLER(open_by_handle_at_x_extra_tail_1, true) {
	...
	if(f == NULL) {
		 ...
		 * Returning an error code means that we drop the entire event, but please note that this
		 * should never happen since we previosuly check `retval > 0`. The kernel should always have
		 * an entry for this fd in the fd table.
		 */
		return PPM_FAILURE_BUG;
	}
...
FILLER(sys_lseek_e, true) {
	/* Parameter 1: fd (type: PT_FD) */
	unsigned long val = bpf_syscall_get_argument(data, 0);
	int64_t fd = (int64_t)(int32_t)val;
	int res = bpf_push_s64_to_ring(data, fd);
	CHECK_RES(res);
...
FILLER(sys_eventfd_e, true) {
	...
	uint64_t initval = (uint64_t)bpf_syscall_get_argument(data, 0);
	int res = bpf_push_u64_to_ring(data, initval);
	CHECK_RES(res);
...
FILLER(sys_eventfd_x, true) {
	...
	uint64_t initval = (uint64_t)bpf_syscall_get_argument(data, 0);
	res = bpf_push_u64_to_ring(data, initval);
	CHECK_RES(res);
 ...
 */
static __always_inline pid_t find_new_reaper_pid(struct filler_data *data,
...
#pragma unroll MAX_HIERARCHY_TRAVERSE
	for(struct task_struct *possible_reaper = (struct task_struct *)_READ(father->real_parent);
		...
		bpf_probe_read_kernel(data->tmp_scratch,
...
FILLER(sys_signaldeliver_e, false) {
	...
	} else if(sig == SIGTERM || sig == SIGHUP || sig == SIGINT || sig == SIGTSTP ||
	          sig == SIGQUIT) {
...
FILLER(sys_setresuid_x, true) {
	...
	uint32_t ruid = (uint32_t)bpf_syscall_get_argument(data, 0);
	res = bpf_push_u32_to_ring(data, ruid);
	CHECK_RES(res);

	/* Parameter 3: euid (type: PT_UINT32) */
	uint32_t euid = (uint32_t)bpf_syscall_get_argument(data, 1);
	res = bpf_push_u32_to_ring(data, euid);
	CHECK_RES(res);

	/* Parameter 4: suid (type: PT_UINT32) */
	uint32_t suid = (uint32_t)bpf_syscall_get_argument(data, 2);
...
FILLER(sched_prog_exec, false) {
	...
	if(args_len && correctly_read == 0) {
		...
		int exe_len = bpf_probe_read_kernel_str(
		        &data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF],
		        SCRATCH_SIZE_HALF,
		        &data->buf[data->state->tail_ctx.curoff & SCRATCH_SIZE_HALF]);
...
#ifndef BPF_SUPPORTS_RAW_TRACEPOINTS
	/* ctx->filename is declared as 32-bit integer, but it is actually defined as
	 * "__data_loc char[] filename;". This means that the 32-bits encodes the length
	 * (upper 16 bits) and the offset from the ctx begin (lower 16 bits). */
	unsigned long filename_offset = (unsigned long)(ctx->filename & 0xFFFF);
	filename_pointer = (unsigned long)original_context + filename_offset;
#else
	struct linux_bprm *bprm = original_context->bprm;
	struct filename *filename = _READ(bprm->filename);
	filename_pointer = (unsigned long)_READ(filename->uptr);
#endif
	return bpf_val_to_ring_mem(data, filename_pointer, USER);
}
...
FILLER(sched_prog_fork, false) {
	 ...
	 */
	struct sched_process_fork_raw_args *original_ctx =
	        (struct sched_process_fork_raw_args *)data->ctx;
...
FILLER(sys_process_vm_readv_x, true) {
	...
	if(retval > 0) {
		...
		res = bpf_parse_readv_writev_bufs(data, iov, iovcnt, retval, PRB_FLAG_PUSH_DATA);
	} else {
		res = bpf_push_empty_param(data);
	}
