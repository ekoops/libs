...
int iter_task(struct bpf_iter__task_file *ctx) {
	...
	/*TODO: this doesn't work. We should use bpf_copy_from_user_task instead (requiring to update
	SEC with iter.s/...)
	// `arg_start` points to the memory area where arguments start. We directly read charbufs from
	// there, not pointers to charbufs! We will store charbufs directly from memory.
	...
	bpf_printk("arg_start_pointer: %lu, arg_end_pointer: %lu", arg_start_pointer, arg_end_pointer);
	...
	// warn: no need to export what in scap_threadinfo is called "flags", as these can safely being
	// inferred in userspace with trivial logic on tgid and pid.
	...
	if(fs != NULL) {
		auxmap__store_d_path_approx(auxmap, &fs->root);
	} else {
		auxmap__store_empty_param(auxmap);
	}

	// warn: filtered_out, fdlist, clone_ts not implemented yet
...
/*static __always_inline int handle_inet_socket_strict(struct seq_file *seq,
                                                 uint32_t pid,
                                                 uint32_t fd,
                                                 struct sock *sk,
                                                 uint16_t sk_type,
    ...
    case SOCK_STREAM:
        if(sk_proto == IPPROTO_TCP || sk_proto == IPPROTO_IP) {
            return handle_inet_socket(seq, pid, fd, sk, sk_type, sk_proto, inode);
        }
        break;
    case SOCK_DGRAM:
...
static __always_inline int handle_inet6_socket_strict(struct seq_file *seq,
                                                  uint32_t pid,
                                                  uint32_t fd,
                                                  struct sock *sk,
                                                  uint16_t sk_type,
    ...
    case SOCK_STREAM:
        if(sk_proto == IPPROTO_TCP || sk_proto == IPPROTO_IP) {
            return handle_inet6_socket(seq, pid, fd, sk, sk_type, sk_proto, inode);
        }
        break;
    case SOCK_DGRAM:
        if(sk_proto == IPPROTO_UDP || sk_proto == IPPROTO_IP) {
...
static __always_inline int handle_inet_socket(struct seq_file *seq,
                                              uint32_t pid,
                                              uint32_t fd,
                                              struct sock *sk,
                                              uint16_t sk_type,
...
static __always_inline int handle_inet6_socket(struct seq_file *seq,
                                               uint32_t pid,
                                               uint32_t fd,
                                               struct sock *sk,
                                               uint16_t sk_type,
	...
	auxmap__store_u16_param(auxmap, sk_type);
	auxmap__store_u16_param(auxmap, sk_proto);
	auxmap__store_u64_param(auxmap, *((uint64_t *)&local_ip));
	auxmap__store_u64_param(auxmap, *((uint64_t *)&local_ip + 1));
	auxmap__store_u16_param(auxmap, local_port);
	auxmap__store_u64_param(auxmap, *((uint64_t *)&remote_ip));
	auxmap__store_u64_param(auxmap, *((uint64_t *)&remote_ip + 1));
	auxmap__store_u16_param(auxmap, remote_port);
	auxmap__store_u64_param(auxmap, ino_num);
...
static __always_inline int handle_unix_socket(struct seq_file *seq,
                                              uint32_t pid,
                                              uint32_t fd,
                                              struct sock *sk,
                                              uint16_t sk_type,
...
static int handle_socket(struct seq_file *seq,
                         uint32_t pid,
                         uint32_t fd,
                         struct file *file,
                         struct inode *inode) {
	...
	case AF_UNIX:
		return handle_unix_socket(seq, pid, fd, sk, sk_type, sk_proto, inode);
	}
	return 0;
}

static int handle_pipe(struct seq_file *seq,
                       uint32_t pid,
                       uint32_t fd,
                       struct file *file,
                       struct inode *inode) {
...
static int handle_memfd_file(struct seq_file *seq,
                             uint32_t pid,
                             uint32_t fd,
                             struct file *file,
                             struct inode *inode) {
...
static int handle_anon_inode_file(struct seq_file *seq,
                                  uint32_t pid,
                                  uint32_t fd,
                                  struct file *file,
                                  struct inode *inode) {
...
static int handle_regular_or_device_file(struct seq_file *seq,
                                         uint32_t pid,
                                         uint32_t fd,
                                         struct file *file,
                                         struct inode *inode) {
	...
	if(fs_magic == ANON_INODE_FS_MAGIC) {
		return handle_anon_inode_file(seq, pid, fd, file, inode);
	}
...
static int handle_directory(struct seq_file *seq,
                            uint32_t pid,
                            uint32_t fd,
                            struct file *file,
                            struct inode *inode) {
...
int iter_task_file(struct bpf_iter__task_file *ctx) {
	...
	case S_IFIFO:
		return handle_pipe(seq, pid, fd, file, inode);
	case S_IFREG:
		if(extract__exe_from_memfd(file)) {
			return handle_memfd_file(seq, pid, fd, file, inode);
		}
		/* fall through */
	case S_IFBLK:
	case S_IFCHR:
	case S_IFLNK:
		return handle_regular_or_device_file(seq, pid, fd, file, inode);
	case S_IFDIR:
		return handle_directory(seq, pid, fd, file, inode);
	case S_IFSOCK:
		return handle_socket(seq, pid, fd, file, inode);
	default:
	...
	return handle_unsupported_file(seq, pid, fd, file, inode, i_mode);
}
