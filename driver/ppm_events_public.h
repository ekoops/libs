...
typedef enum {
	...
	PPME_ITER_TASK_E = 430,
	PPME_ITER_TASK_X = 431,
	PPME_ITER_TASK_FILE_SOCKET_INET_E = 432,
	PPME_ITER_TASK_FILE_SOCKET_INET_X = 433,
	PPME_ITER_TASK_FILE_SOCKET_INET6_E = 434,
	PPME_ITER_TASK_FILE_SOCKET_INET6_X = 435,
	PPME_ITER_TASK_FILE_SOCKET_UNIX_E = 436,
	PPME_ITER_TASK_FILE_SOCKET_UNIX_X = 437,
	PPME_ITER_TASK_FILE_SOCKET_NETLINK_E = 438,
	PPME_ITER_TASK_FILE_SOCKET_NETLINK_X = 439,
	PPME_ITER_TASK_FILE_PIPE_E = 440,
	PPME_ITER_TASK_FILE_PIPE_X = 441,
	PPME_ITER_TASK_FILE_DIRECTORY_E = 442,
	PPME_ITER_TASK_FILE_DIRECTORY_X = 443,
	PPME_ITER_TASK_FILE_REGULAR_E = 444,
	PPME_ITER_TASK_FILE_REGULAR_X = 445,
	PPME_ITER_TASK_FILE_ANON_INODE_E = 446,
	PPME_ITER_TASK_FILE_ANON_INODE_X = 447,
	PPME_ITER_TASK_FILE_MEMFD_E = 448,
	PPME_ITER_TASK_FILE_MEMFD_X = 449,
	PPM_EVENT_MAX = 450
} ppm_event_code;
/*@}*/
...
enum ppm_event_category {
	...
	EC_METAEVENT = (1 << 12),  /* Meta-event not generated by a source but used for notifications or
	                              enrichment */
	EC_ITER = (1 << 13),       /* Event generated by an iterator */
};

enum ppm_event_flags {
	EF_NONE = 0,
	EF_CREATES_FD =
	        (1 << 0), /* This event creates an FD (e.g. open). NOTE: a parser MUST always be created
	                     when this flag is set, to parse fd and add it to threadinfo list of fds */
	EF_DESTROYS_FD = (1 << 1), /* This event destroys an FD (e.g. close). NOTE: a parser MUST always
	                              be created when this flag is set, to parse fd and erasing it from
	                              threadinfo list (using sinsp_parser::erase_fd) */
	...
	EF_MODIFIES_STATE = (1 << 5), /* This event causes the machine state to change and should not be
	                                 dropped by the filtering engine. */
	EF_UNUSED = (1 << 6),         /* This event is not used */
	EF_WAITS = (1 << 7),          /* This event reads data from an FD. */
	EF_SKIPPARSERESET =
	        (1 << 8), /* This event shouldn't pollute the parser lastevent state tracker. */
	EF_OLD_VERSION = (1 << 9), /* This event is kept for backward compatibility */
	// EF_DROP_SIMPLE_CONS = (1 << 10), /* This event can be skipped by consumers that privilege low
...
enum ppm_param_type {
	...
	PT_FSPATH =
	        17, /* A string containing a relative or absolute file system path, null terminated */
	PT_SYSCALLID =
	        18, /* A 16bit system call ID. Can be used as a key for the g_ppm_sc_names table. */
	PT_SIGTYPE = 19, /* An 8bit signal number */
	PT_RELTIME = 20, /* A relative time. Seconds * 10^9  + nanoseconds. 64bit. */
	PT_ABSTIME =
	        21,   /* An absolute time interval. Seconds from epoch * 10^9  + nanoseconds. 64bit. */
	...
	PT_CHARBUFARRAY = 35, /* Pointer to an array of strings, exported by the user events decoder.
	                         64bit. */
	PT_CHARBUF_PAIR_ARRAY = 36, /* Pointer to an array of string pairs, exported by the user events
	...
	PT_ENUMFLAGS8 = 44, /* this is an UINT8, but will be interpreted as an enum flag, ie: contiguous
	                       values flag. */
	PT_ENUMFLAGS16 = 45, /* this is an UINT16, but will be interpreted as an enum flag, ie:
...
enum ppm_print_format {
	...
	PF_10_PADDED_DEC = 3, /* decimal padded to 10 digits, useful to print the fractional part of a
	                         ns timestamp */
	PF_ID = 4,
	PF_DIR = 5,
	PF_OCT = 6, /* octal */
...
struct ppm_param_info {
	...
	enum ppm_print_format fmt; /**< If this is a numeric parameter, this flag specifies if it should
	                              be rendered as decimal or hex. */
	const void
	        *info; /**< If this is a flags parameter, it points to an array of ppm_name_value,
...
#define PPM_IOCTL_DISABLE_SYSCALL \
	_IO(PPM_IOCTL_MAGIC, 30)  // this replaces PPM_IOCTL_MASK_UNSET_EVENT
...
enum syscall_flags {
	...
	UF_NEVER_DROP = (1 << 1),
	UF_ALWAYS_DROP = (1 << 2),
	// UF_SIMPLEDRIVER_KEEP = (1 << 3), ///< Mark a syscall to be kept in simpledriver mode, see
	// scap_enable_simpledriver_mode() -> SUPPORT DROPPED
 ...
 * in going beyond 0xffff. Notice: this is less than 2^16 - 1 for historical reasons, linked to the
 * fact that this value was also used in the legacy BPF probe: the limit there was more stringent,
 * because the entire perf event had to fit in uint16_t. Keep it as is for the moment, as this
 * shouldn't cause any issues since typically snaplen is much lower than this.
 */
...
enum anon_inode_fd_type {
	ANON_INODE_FD_TYPE_UNKNOWN = 0,
	ANON_INODE_FD_TYPE_EVENTFD = 1,
	ANON_INODE_FD_TYPE_EVENTPOLL = 2,
	ANON_INODE_FD_TYPE_INOTIFY = 3,
	ANON_INODE_FD_TYPE_SIGNALFD = 4,
	ANON_INODE_FD_TYPE_TIMERFD = 5,
	ANON_INODE_FD_TYPE_IO_URING = 6,
	ANON_INODE_FD_TYPE_USERFAULTFD = 7,
	ANON_INODE_FD_TYPE_PIDFD = 8,
	ANON_INODE_FD_TYPE_BPF_MAP = 9,
	ANON_INODE_FD_TYPE_BPF_PROG = 10,
	ANON_INODE_FD_TYPE_BPF_LINK = 11,
	ANON_INODE_FD_TYPE_BPF_ITER = 12,
	ANON_INODE_FD_TYPE_PERF_EVENT = 13,
};

// Maximum size of an event generated by an iterator.
#define MAX_ITER_EVENT_SIZE 32 * 1024
