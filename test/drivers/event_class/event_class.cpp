...
static_assert(sizeof(cgroup_prefix_array) / sizeof(*cgroup_prefix_array) == CGROUP_NUMBER,
...
void _assert_syscall_state(int syscall_state,
	...
	if(!match) {
		if(syscall_state == SYSCALL_SUCCESS) {
			FAIL() << ">>>>> The syscall '" << syscall_name
			       << "' must be successful. Errno: " << errno
			       << " err_message: " << strerror(errno) << std::endl;
		} else {
 ...
 */
event_test::event_test(ppm_sc_code sc_code): m_sc_set(PPM_SC_MAX, 0) {
	m_current_param = 0;
...
event_test::event_test(int syscall_id, int event_direction): m_sc_set(PPM_SC_MAX, 0) {
 ...
 */
event_test::event_test(): m_sc_set(PPM_SC_MAX, 0) {
	m_current_param = 0;
...
void event_test::parse_event() {
	 ...
	 * Note: we compute this length according to the number of params written in the header by the
	 * bpf program.
	 */
...
void event_test::server_fill_sockaddr_in(sockaddr_in* sockaddr,
                                         int32_t ipv4_port,
                                         const char* ipv4_string) {
	memset(sockaddr, 0, sizeof(*sockaddr));
	sockaddr->sin_family = AF_INET;
...
void event_test::client_fill_sockaddr_in6(sockaddr_in6* sockaddr,
                                          int32_t ipv6_port,
...
void event_test::server_fill_sockaddr_in6(sockaddr_in6* sockaddr,
                                          int32_t ipv6_port,
                                          const char* ipv6_string) {
	memset(sockaddr, 0, sizeof(*sockaddr));
	sockaddr->sin6_family = AF_INET6;
...
void event_test::connect_ipv4_client_to_server(int32_t* client_socket,
	...
	*server_socket = syscall(__NR_socket, AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0);
	...
	assert_syscall_state(SYSCALL_SUCCESS,
	                     "listen (server)",
	                     syscall(__NR_listen, *server_socket, QUEUE_LENGTH),
	                     NOT_EQUAL,
	                     -1);
...
void event_test::client_to_server(send_data send_d,
	...
	case protocol_L3::IPv4:
		if(net_config.proto_L4 == protocol_L4::TCP) {
			this->connect_ipv4_client_to_server(&client_socket_fd,
			                                    &client_addr,
			                                    &server_socket_fd,
			                                    &server_addr,
			                                    net_config.client_port,
			                                    net_config.server_port);
		} else {
			this->connect_ipv4_udp_client_to_server(&client_socket_fd,
			                                        &client_addr,
			                                        &server_socket_fd,
			                                        &server_addr,
			                                        net_config.client_port,
		...
		// for the `recv*` syscalls we will use the memory of the server sockaddr but this will be
		// overwritten by the kernel so it shouldn't be an issue.
		addr = (sockaddr*)&server_addr;
		addrlen = sizeof(server_addr);
		break;
	case protocol_L3::IPv6:
		if(net_config.proto_L4 == protocol_L4::TCP) {
			this->connect_ipv6_client_to_server(&client_socket_fd,
			                                    &client_addr6,
			                                    &server_socket_fd,
			                                    &server_addr6,
			                                    net_config.client_port,
			...
			this->connect_ipv6_udp_client_to_server(&client_socket_fd,
			                                        &client_addr6,
			                                        &server_socket_fd,
			                                        &server_addr6,
			                                        net_config.client_port,
	...
	case __NR_sendmsg: {
		...
		assert_syscall_state(SYSCALL_SUCCESS,
		                     "sendmsg (client)",
		                     syscall(__NR_sendmsg, client_socket_fd, &send_msg, sendmsg_flags),
		                     NOT_EQUAL,
		                     -1);
	} break;
	...
	case __NR_recvfrom: {
		...
		int64_t received_bytes = syscall(__NR_recvfrom,
		                                 receive_socket_fd,
		                                 received_data_ptr,
		                                 received_data_len,
		                                 recvfrom_flags,
	...
	case __NR_read: {
		...
		ssize_t read_bytes =
		        syscall(__NR_read, receive_socket_fd, (void*)received_data_ptr, received_data_len);
...
void event_test::connect_ipv4_udp_client_to_server(int32_t* client_socket,
                                                   sockaddr_in* client_sockaddr,
                                                   int32_t* server_socket,
                                                   sockaddr_in* server_sockaddr,
                                                   int32_t port_client,
...
void event_test::connect_ipv6_client_to_server(int32_t* client_socket,
...
void event_test::connect_ipv6_udp_client_to_server(int32_t* client_socket,
                                                   sockaddr_in6* client_sockaddr,
                                                   int32_t* server_socket,
                                                   sockaddr_in6* server_sockaddr,
                                                   int32_t port_client,
...
void event_test::assert_header() {
	/* TODO: Here we need a `scap` function that exposes some fields of the table and not all the
	 * table!! */
	...
	ASSERT_EQ(m_event_header->nparams, num_params_from_bpf_table)
	        << "'nparams' in the header is not correct." << std::endl;
	/* the len specified in the header matches the real event len. */
	ASSERT_EQ(m_event_header->len, m_event_len)
	        << "'event_len' in the header is not correct." << std::endl;
}
...
void event_test::assert_num_params_pushed(int total_params) {
	 ...
	 * table!! */
	int num_params_from_bpf_table = scap_get_event_info_table()[m_event_type].nparams;
	ASSERT_EQ(total_params, num_params_from_bpf_table)
	        << "for this event we have not pushed the right number of parameters." << std::endl;
}
...
void event_test::assert_numeric_param(int param_num, T param, assertion_operators op) {
	...
	switch(op) {
	case EQUAL:
		ASSERT_EQ(*(T*)(m_event_params[m_current_param].valptr), param)
		        << VALUE_NOT_CORRECT << m_current_param << std::endl;
		break;

	case NOT_EQUAL:
		ASSERT_NE(*(T*)(m_event_params[m_current_param].valptr), param)
		        << VALUE_NOT_CORRECT << m_current_param << std::endl;
		break;

	case GREATER_EQUAL:
		ASSERT_GE(*(T*)(m_event_params[m_current_param].valptr), param)
		        << VALUE_NOT_CORRECT << m_current_param << std::endl;
		break;

	case LESS_EQUAL:
		ASSERT_LE(*(T*)(m_event_params[m_current_param].valptr), param)
		        << VALUE_NOT_CORRECT << m_current_param << std::endl;
		break;
...
void event_test::assert_charbuf_array_param(int param_num, const char** param) {
	...
	for(int index = 0; index < MAX_CHARBUF_NUM; index++) {
		...
		ASSERT_EQ(strlen(m_event_params[m_current_param].valptr + total_len), strlen(param[index]))
		        << LEN_NOT_CORRECT << m_current_param << std::endl;
		ASSERT_STREQ(m_event_params[m_current_param].valptr + total_len, param[index])
		        << VALUE_NOT_CORRECT << m_current_param << std::endl;
...
void event_test::assert_cgroup_param(int param_num) {
	...
	for(int index = 0; index < CGROUP_NUMBER; index++) {
		strlcpy(cgroup_string,
		        m_event_params[m_current_param].valptr + total_len,
		        MAX_CGROUP_STRING_LEN);
		...
		ASSERT_STREQ(cgroup_prefix, cgroup_prefix_array[index])
...
void event_test::assert_bytebuf_param(int param_num, const char* param, int buf_dimension) {
	...
	ASSERT_EQ(memcmp(m_event_params[m_current_param].valptr, param, buf_dimension), 0)
	        << VALUE_NOT_CORRECT << m_current_param << std::endl;
}

void event_test::assert_addr_info_inet_param(int param_num,
...
void event_test::assert_addr_info_inet6_param(int param_num,
                                              uint8_t desired_family,
...
void event_test::assert_addr_info_unix_param(int param_num,
                                             uint8_t desired_family,
...
void event_test::assert_tuple_inet_param(int param_num,
                                         uint8_t desired_family,
...
void event_test::assert_tuple_inet6_param(int param_num,
                                          uint8_t desired_family,
...
void event_test::assert_tuple_unix_param(int param_num,
                                         uint8_t desired_family,
                                         const char* desired_path) {
	assert_param_boundaries(param_num);
...
void event_test::assert_setsockopt_val(int param_num,
                                       int sockopt,
                                       void* option_value,
                                       int option_len) {
	...
	ASSERT_EQ(*(uint8_t*)(m_event_params[m_current_param].valptr), sockopt)
	        << VALUE_NOT_CORRECT << m_current_param << std::endl;
	...
	case PPM_SOCKOPT_IDX_TIMEVAL:
		ASSERT_EQ(*(uint64_t*)(m_event_params[m_current_param].valptr + 1),
	...
	case PPM_SOCKOPT_IDX_UINT32:
		ASSERT_EQ(*(uint32_t*)(m_event_params[m_current_param].valptr + 1),
...
void event_test::assert_ptrace_addr(int param_num) {
	...
	ASSERT_EQ(*(uint8_t*)(m_event_params[m_current_param].valptr), PPM_PTRACE_IDX_UINT64)
	        << VALUE_NOT_CORRECT << m_current_param << std::endl;
	ASSERT_EQ(*(uint64_t*)(m_event_params[m_current_param].valptr + 1), 0)
	        << VALUE_NOT_CORRECT << m_current_param << std::endl;
}
...
void event_test::assert_fd_list(int param_num,
                                struct fd_poll* expected_fds,
                                int32_t expected_nfds) {
	assert_param_boundaries(param_num);
	uint16_t bytes_read = 0;

	/* Assert the pair's number */
	ASSERT_EQ(*(int16_t*)(m_event_params[m_current_param].valptr + bytes_read), expected_nfds)
	        << VALUE_NOT_CORRECT << m_current_param << " the expected number of nfds doesn't match!"
	        << std::endl;
	bytes_read += sizeof(uint16_t);

	for(int j = 0; j < expected_nfds; j++) {
		/* Assert the `fd` */
		ASSERT_EQ(*(int64_t*)(m_event_params[m_current_param].valptr + bytes_read),
		          expected_fds[j].fd)
		        << VALUE_NOT_CORRECT << m_current_param << " index: " << j << std::endl;
		bytes_read += sizeof(int64_t);
		/* Assert `flags` */
		ASSERT_EQ(*(int16_t*)(m_event_params[m_current_param].valptr + bytes_read),
		          expected_fds[j].flags)
		        << VALUE_NOT_CORRECT << m_current_param << " index: " << j << std::endl;
...
void event_test::assert_param_len(uint16_t expected_size) {
	...
	ASSERT_EQ(size, expected_size)
	        << ">>>>> length of the param is not correct. Param id = " << m_current_param
	        << std::endl;
}

void event_test::assert_param_len_ge(uint16_t expected_size) {
	uint16_t size = m_event_params[m_current_param].len;
	ASSERT_GE(size, expected_size)
	        << ">>>>> length of the param is not correct. Param id = " << m_current_param
	        << std::endl;
}
...
void event_test::assert_ipv4_string(const char* desired_ipv4, int starting_index, direction dir) {
	...
	if(inet_ntop(AF_INET,
	             (uint8_t*)(m_event_params[m_current_param].valptr + starting_index),
	             ipv4_string,
	             ADDRESS_LENGTH) == NULL) {
	...
	if(dir == DEST) {
		ASSERT_STREQ(ipv4_string, desired_ipv4)
		        << VALUE_NOT_CORRECT << m_current_param << " (dest ipv4)" << std::endl;
	} else {
		ASSERT_STREQ(ipv4_string, desired_ipv4)
		        << VALUE_NOT_CORRECT << m_current_param << " (source ipv4)" << std::endl;
	}
}
...
void event_test::assert_ipv6_string(const char* desired_ipv6, int starting_index, direction dir) {
	...
	if(inet_ntop(AF_INET6,
	             (uint32_t*)(m_event_params[m_current_param].valptr + starting_index),
	             ipv6_string,
	             ADDRESS_LENGTH) == NULL) {
		FAIL() << "'inet_ntop' must not fail. Param id = " << m_current_param << std::endl;
	}
	...
	if(dir == DEST) {
		ASSERT_STREQ(ipv6_string, desired_ipv6)
		        << VALUE_NOT_CORRECT << m_current_param << "(dest ipv6)" << std::endl;
	} else {
		ASSERT_STREQ(ipv6_string, desired_ipv6)
		        << VALUE_NOT_CORRECT << m_current_param << "(source ipv6)" << std::endl;
	}
}
	...
	while(true) {
		...
		if(m_event_header->tid == (uint64_t)pid && m_event_header->type == m_event_type) {
			if(presence) {
				...
				FAIL() << "There is an event '" << m_event_type
				       << "' in the buffers, but it shouldn't be there" << std::endl;
			}
		}
	}
}
