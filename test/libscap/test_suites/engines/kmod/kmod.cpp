...
int remove_kmod(char* error_buf) {
	if(syscall(__NR_delete_module, LIBSCAP_TEST_KERNEL_MODULE_NAME, O_NONBLOCK)) {
		...
		case EPERM:
			snprintf(error_buf,
			         SCAP_LASTERR_SIZE,
			         "Unable to remove kernel module. Errno message: %s, errno: %d\n",
			         strerror(errno),
			         errno);
			return EXIT_FAILURE;
		...
		default:
			snprintf(error_buf,
			         SCAP_LASTERR_SIZE,
			         "Unexpected error code. Errno message: %s, errno: %d\n",
			         strerror(errno),
			         errno);
			return EXIT_FAILURE;
		}
...
int insert_kmod(const char* kmod_path, char* error_buf) {
	...
	if(fd < 0) {
		snprintf(error_buf,
		         SCAP_LASTERR_SIZE,
		         "Unable to open the kmod file. Errno message: %s, errno: %d\n",
		         strerror(errno),
		         errno);
		return EXIT_FAILURE;
	}
	...
	if(syscall(__NR_finit_module, fd, "", 0)) {
		snprintf(error_buf,
		         SCAP_LASTERR_SIZE,
		         "Unable to inject the kmod. Errno message: %s, errno: %d\n",
		         strerror(errno),
		         errno);
		return EXIT_FAILURE;
	}
...
scap_t* open_kmod_engine(char* error_buf,
                         int32_t* rc,
...
TEST(kmod, read_in_order) {
	...
	scap_t* h =
	        open_kmod_engine(error_buffer, &ret, 1 * 1024 * 1024, LIBSCAP_TEST_KERNEL_MODULE_PATH);
	ASSERT_FALSE(!h || ret != SCAP_SUCCESS)
	        << "unable to open kmod engine: " << error_buffer << std::endl;
...
TEST(kmod, metrics_v2_check_per_CPU_stats) {
	...
	while(i < nstats) {
		...
		if(strncmp(stats_v2[i].name,
			...
			if(strncmp(stats_v2[i].name,
			           N_DROPS_PER_DEVICE_PREFIX,
			           sizeof(N_DROPS_PER_DEVICE_PREFIX) - 1) == 0) {
				i++;
				found++;
			} else {
	...
	// This test could fail in case of rare race conditions in which the number of online CPUs
	// changes between the scap_open and the `sysconf(_SC_NPROCESSORS_ONLN)` function. In CI we
	// shouldn't have hot plugs so probably we can live with this.
