...
static void test_open_log_fn(const char* component,
                             const char* msg,
                             falcosecurity_log_severity sev) {
...
TEST(modern_bpf, one_buffer_per_possible_CPU) {
	...
	ASSERT_EQ(num_expected_rings, num_possible_CPUs)
	        << "we should have a ring buffer for every possible CPU!" << std::endl;
...
TEST(modern_bpf, one_buffer_every_two_possible_CPUs) {
	...
	ASSERT_EQ(num_rings, num_expected_rings)
	        << "we should have one ring buffer every 2 CPUs!" << std::endl;
 ...
 * we have already done it in the previous test
