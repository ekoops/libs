	...
	*nbytes = 0;
	...
	case SCAP_FD_UNIX_SOCK:
		if(r->read(r, &(fdi->info.unix_socket_info.source), sizeof(uint64_t)) != sizeof(uint64_t) ||
		   r->read(r, &(fdi->info.unix_socket_info.destination), sizeof(uint64_t)) !=
		           sizeof(uint64_t)) {
			scap_errprintf(error, 0, "error reading the fd info from file (fi5)");
		}
	...
	case SCAP_FD_FILE_V2:
		if(r->read(r, &(fdi->info.regularinfo.open_flags), sizeof(uint32_t)) != sizeof(uint32_t)) {
			scap_errprintf(error, 0, "error reading the fd info from file (fi1)");
		}
		...
		if(r->read(r, &(fdi->info.regularinfo.dev), sizeof(uint32_t)) != sizeof(uint32_t)) {
			scap_errprintf(error, 0, "error reading the fd info from file (dev)");
		}
		(*nbytes) += sizeof(uint32_t);
		break;
	case SCAP_FD_FIFO:
...
static int32_t scap_read_fdlist(scap_reader_t *r,
	...
	while(((int32_t)block_length - (int32_t)totreadsize) >= 4) {
		...
		//
		proclist->m_proc_callback(proclist->m_proc_callback_context, tid, NULL, &fdi, NULL, error);
	}
	...
	while(true) {
		...
		if(bh.block_type != EV_BLOCK_TYPE && bh.block_type != EV_BLOCK_TYPE_V2 &&
		   ...
		   bh.block_type != EVF_BLOCK_TYPE_V2_LARGE) {
			scap_errprintf(error, 0, "unexpected block type %u", (uint32_t)bh.block_type);
			handle->m_use_last_block_header = true;
		...
		CHECK_READ_SIZE_ERR(readsize, readlen, error);
...
static int32_t next(struct scap_engine_handle engine,
	...
	int32_t res = next_event_from_file(handle, pevent, pdevid, pflags, error);
	...
	if(conv_num == MAX_CONVERSION_BOUNDARY) {
		...
		if(conv_res == CONVERSION_CONTINUE) {
		...
		return scap_errprintf(error, 0, "Unknown error");
	}
...
static void *alloc_handle(struct scap *main_handle) {
	return calloc(1, sizeof(struct savefile_engine));
}

static int32_t init(struct scap *main_handle, struct scap_open_args *oargs, char *error) {
	gzFile gzfile;
	int res;
	...
	if(gzfile == NULL) {
		if(fd != 0) {
			return scap_errprintf(error, 0, "can't open fd %d", fd);
		}
		return scap_errprintf(error, 0, "can't open file %s", fname);
	}
	...
	if(!handle->m_reader_evt_buf) {
		return scap_errprintf(error, 0, "error allocating the read buffer");
	}
...
static int32_t scap_savefile_close(struct scap_engine_handle engine, char *error) {
	struct savefile_engine *handle = engine.m_handle;
	if(handle->m_reader) {
...
static int32_t scap_savefile_restart_capture(scap_t *handle, char *error) {
	...
	if((res = scap_read_init(engine,
		...
		scap_errprintf(error, 0, "could not restart capture: %s", error_copy);
	}
	return res;
}
