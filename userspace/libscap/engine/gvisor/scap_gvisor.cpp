...
int32_t engine::init(std::string config_path,
...
int32_t engine::start_capture(char *error) {
	...
	for(const auto &sandbox : existing_sandboxes) {
		...
		runsc::result trace_create_res =
		        runsc::trace_create(m_root_path, m_trace_session_path, sandbox, true);
	...
	for(const auto &sandbox : new_sandboxes) {
		runsc::result trace_create_res =
		        runsc::trace_create(m_root_path, m_trace_session_path, sandbox, false);
		if(trace_create_res.error) {
			// some sandboxes may not be traced, we can skip them safely
			continue;
		}
...
const metrics_v2 *engine::get_stats_v2(uint32_t flags, uint32_t *nstats, int32_t *rc, char *error) {
	*nstats = scap_gvisor::stats::MAX_GVISOR_COUNTERS_STATS;
	...
	stats[scap_gvisor::stats::GVISOR_N_DROPS_BUFFER_TOTAL].value.u64 =
	        m_gvisor_stats.n_drops_parsing + m_gvisor_stats.n_drops_gvisor;
...
// Reads one gvisor message from the specified fd, stores the resulting events overwriting m_buffers
// and adds pointers to m_event_queue. Returns:
// * SCAP_SUCCESS in case of success
// * SCAP_FAILURE in case of a fatal error while reading from the fd or allocating memory (m_lasterr
// is filled)
...
int32_t engine::process_message_from_fd(int fd, char *error) {
	...
	scap_const_sized_buffer gvisor_msg = {.buf = static_cast<void *>(message),
	                                      .size = static_cast<size_t>(nbytes)};
	...
	parsers::parse_result parse_result =
