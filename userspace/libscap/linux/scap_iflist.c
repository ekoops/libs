	...
	for(tempIfAddr = interfaceArray; tempIfAddr != NULL; tempIfAddr = tempIfAddr->ifa_next) {
		...
		if(tempIfAddr->ifa_addr->sa_family == AF_INET) {
			...
			if(tempIfAddr->ifa_netmask != NULL) {
				addrlist->v4list[ifcnt4].netmask =
				        *(uint32_t *)&(((struct sockaddr_in *)tempIfAddr->ifa_netmask)->sin_addr);
			} else {
				addrlist->v4list[ifcnt4].netmask = 0;
			}

			if(tempIfAddr->ifa_ifu.ifu_broadaddr != NULL) {
				addrlist->v4list[ifcnt4].bcast = *(uint32_t *)&(
				        ((struct sockaddr_in *)tempIfAddr->ifa_ifu.ifu_broadaddr)->sin_addr);
			} else {
				addrlist->v4list[ifcnt4].bcast = 0;
			}
			strlcpy(addrlist->v4list[ifcnt4].ifname,
			        tempIfAddr->ifa_name,
			        sizeof(addrlist->v4list[ifcnt4].ifname));
			addrlist->v4list[ifcnt4].ifnamelen = strlen(tempIfAddr->ifa_name);

			addrlist->v4list[ifcnt4].linkspeed = 0;

			addrlist->totlen += (sizeof(scap_ifinfo_ipv4) + addrlist->v4list[ifcnt4].ifnamelen -
			                     SCAP_MAX_PATH_SIZE);
			ifcnt4++;
		} else if(tempIfAddr->ifa_addr->sa_family == AF_INET6) {
			...
			strlcpy(addrlist->v6list[ifcnt6].ifname,
			        tempIfAddr->ifa_name,
			        sizeof(addrlist->v6list[ifcnt6].ifname));
			addrlist->v6list[ifcnt6].ifnamelen = strlen(tempIfAddr->ifa_name);

			addrlist->v6list[ifcnt6].linkspeed = 0;

			addrlist->totlen += (sizeof(scap_ifinfo_ipv6) + addrlist->v6list[ifcnt6].ifnamelen -
			                     SCAP_MAX_PATH_SIZE);
			ifcnt6++;
		} else {
			continue;
