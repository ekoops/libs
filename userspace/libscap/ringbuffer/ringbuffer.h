...
static inline void ringbuffer_get_buf_pointers(scap_device* dev,
                                               uint64_t* phead,
                                               uint64_t* ptail,
                                               uint64_t* pread_size) {
 ...
 *
 * The flow here is:
 * - For every buffer, read how many data are available and save the pointer + its length. (this is
 * what we call a block)
 * - Consume from all these blocks the event with the lowest timestamp. (repeat until all the blocks
 * are empty!) When we have read all the data from a buffer block, update the consumer position for
 * that buffer, and wait for all the other buffer blocks to be read.
