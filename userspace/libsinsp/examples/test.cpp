...
void parse_CLI_options(sinsp& inspector, int argc, char** argv) {
	...
	options.add_options()
		...
		("1,threads-iter", "pid to filter", cxxopts::value<int>())
	...
	try {
		...
		if(result.count("threads-iter")) {
			threads_iter_pid = result["threads-iter"].as<int>();
...
int main(int argc, char** argv) {
	...
	if(use_threads_iter) {
		inspector.get_threads(threads_iter_pid);
	}

	if(use_files_iter) {
		inspector.get_files(files_iter_pid);
	}
