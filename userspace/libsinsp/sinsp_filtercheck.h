...
public:
	...
	//
	virtual const filter_check_info* get_fields() const { return m_info; }

	//
	// Return the info about the field that this instance contains
	// This must be used only after `parse_field_name`
	//
	virtual const filtercheck_field_info* get_field_info() const { return m_field; }
	...
	//
	virtual const std::vector<filter_value_t>& get_filter_values() const { return m_vals; }

	//
	// Return true if the filter check is compared against another filter check
	//
	virtual bool has_filtercheck_value() const { return m_rhs_filter_check.get() != nullptr; }
	...
	//
	virtual bool has_transformers() const { return !m_transformers.empty(); }
...
protected:
	...
	virtual size_t parse_filter_value(const char* str,
	                                  uint32_t len,
	                                  uint8_t* storage,
