...
bool sinsp::is_initialstate_event(const scap_evt& pevent) {
	...
	return evt_type == PPME_CONTAINER_JSON_2_E || evt_type == PPME_USER_ADDED_E ||
...
void sinsp::try_open_common(scap_open_args* oargs,
                            const scap_vtable* vtable,
                            scap_platform* platform,
                            const sinsp_mode_t mode) {
	try {
		open_common(oargs, vtable, platform, mode);
	} catch(...) {
		// note: platform is allocated into the caller, while the scap handle is allocated inside
		// `open_common()`.
...
void sinsp::open_kmod(unsigned long driver_buffer_bytes_dim,
	...
	try_open_common(&oargs, &scap_kmod_engine, platform, SINSP_MODE_LIVE);
#else
...
void sinsp::open_nodriver(bool full_proc_scan) {
	...
	try_open_common(&oargs, &scap_nodriver_engine, platform, SINSP_MODE_NODRIVER);
#else
...
void sinsp::open_savefile(const std::string& filename, int fd) {
	...
	try_open_common(&oargs, &scap_savefile_engine, platform, SINSP_MODE_CAPTURE);
#else
...
void sinsp::open_plugin(const std::string& plugin_name,
	...
	try_open_common(&oargs, &scap_source_plugin_engine, platform, mode);
#else
...
void sinsp::get_threads(const int pid) const {
	auto* platform = reinterpret_cast<struct scap_linux_platform*>(m_platform);
	platform->m_linux_vtable->get_threads(m_h->m_engine, pid, nullptr);
}

void sinsp::get_files(const int pid) const {
	auto* platform = reinterpret_cast<struct scap_linux_platform*>(m_platform);
	platform->m_linux_vtable->get_files(m_h->m_engine, pid, nullptr);
}
...
void sinsp::open_test_input(scap_test_input_data* data, sinsp_mode_t mode) {
	...
	try_open_common(&oargs, &scap_test_input_engine, platform, mode);
