...
static int size_iter_auxiliary_maps(const struct bpf_probe* probe, const uint32_t max_entries) {
	if(bpf_map__set_max_entries(probe->maps.iter_auxiliary_maps, max_entries)) {
		pman_print_errorf("unable to set max entries for 'iter_auxiliary_maps' to %d", max_entries);
		return errno;
	}
	return 0;
}
...
int pman_prepare_maps_before_loading() {
	...
	err = err ?: size_iter_auxiliary_maps(g_state.skel, g_state.n_possible_cpus);
...
int iter_support_probing__prepare_maps_before_loading(struct iter_support_probing_ctx* ctx) {
	...
	err = err ?: size_iter_auxiliary_maps(ctx->probe, 1);
