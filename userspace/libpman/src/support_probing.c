...
static int init_ctx(struct iter_support_probing_ctx *ctx) {
...
static void destroy_ctx(struct iter_support_probing_ctx *ctx) {
	if(ctx->probe) {
...
static void disable_all_progs(struct bpf_probe *probe) {
	...
	// (note: disabling creation of these maps don't work in these cases).
